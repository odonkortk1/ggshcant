...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (payment === "success") {
      ...
      if (reference) {
        base44.functions.invoke("paystack-verify", { reference }).
          then((res) => {
            if (res.data?.success) {
  ...
  const placeOrder = async ({ customer_name, pickup_note, momoSuccess, cashPayment, ggshPayment, client_phone }) => {
    ...
    if (window.self !== window.top) {
      toast({
        ...
        description: "Please open the published app to complete payment.",
        variant: "destructive",
      });
      return;
    }
      ...
      <header className="sticky top-0 z-30 bg-background/80 backdrop-blur-md border-b">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="font-heading font-bold text-[15px] leading-none">GGSHCanteen</h1>
              ...
              <>
                ...
                <Link to="/orders">
                  <Button variant="outline" size="sm" className="rounded-full">
                    <ClipboardList className="w-4 h-4 mr-1.5" /> Orders
                  </Button>
                </Link>
              </>
            )}
            {!isAdmin && (
              <Link to="/staff-login">
                <Button variant="ghost" size="sm" className="rounded-full text-muted-foreground">
                  <ClipboardList className="w-4 h-4 mr-1.5" /> Staff
                </Button>
              </Link>
            )}
            {client ? (
              <div className="flex items-center gap-1">
                ...
                <Button variant="ghost" size="icon" className="rounded-full" onClick={logoutClient}>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-10 pb-8">
          <div className="relative rounded-3xl overflow-hidden text-white p-8 sm:p-12 min-h-[260px] flex items-center">
            <img
              ...
              alt="Fresh cafeteria meals"
              className="absolute inset-0 w-full h-full object-cover"
            />
            <div className="absolute inset-0 bg-gradient-to-r from-blue-900/90 via-blue-800/75 to-blue-700/40" />
            <div className="relative max-w-lg">
              <span className="inline-block bg-white/15 backdrop-blur-sm text-[11px] font-medium px-3 py-1 rounded-full mb-3">
                Open daily · 6 AM – 8 PM
              </span>
              <h2 className="font-heading text-3xl sm:text-4xl font-bold leading-tight mb-2">
                Fresh meals for our care teams & visitors
              </h2>
              <p className="text-blue-50/90 text-sm leading-relaxed">
                Nourishing, affordable food prepared daily. Place your order and pick it up at the counter — no queue, no waiting.
              </p>
            </div>
          </div>
        </div>
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40">
          <Button
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg h-12 px-6"
          >
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Pickup at the counter. Total ₵{lastOrder.total.toFixed(2)}.
              </p>
              <Button
              variant="link"
              className="h-auto p-0 mt-1 text-xs text-blue-700"
              onClick={() => {setLastOrder(null);setCartOpen(false);}}>
