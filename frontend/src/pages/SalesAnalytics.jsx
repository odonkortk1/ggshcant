...
export default function SalesAnalytics() {
  ...
  const applyPreset = (preset) => {
    ...
    else if (preset === "month") { const m = new Date(now); m.setMonth(m.getMonth(), 1); m.setDate(1); s = toISODate(m); }
  ...
  const momoOrders = completedOrders.filter((o) => o.payment_method === "momo");
  ...
  const statusData = ["pending", "preparing", "ready", "completed"].map((s) => ({
    name: s.charAt(0).toUpperCase() + s.slice(1),
    value: rangeOrders.filter((o) => o.status === s).length,
  ...
  const chartData = useMemo(() => {
    ...
    if (spanDays <= 31) {
      for (let d = new Date(s); d <= e; d.setDate(d.getDate() + 1)) {
        ...
        const dayOrders = orders.filter((o) => o.created_date?.split("T")[0] === dateStr);
        buckets.push({
          label: new Date(d).toLocaleDateString("en", { month: "short", day: "numeric" }),
          momo: dayOrders.filter((o) => o.status === "completed" && o.payment_method === "momo").reduce((s, o) => s + o.total, 0),
      ...
      while (cursor <= e) {
        ...
        const weekOrders = orders.filter((o) => {
          const od = new Date(o.created_date);
          return od >= cursor && od <= bEnd;
        });
        buckets.push({
          label: `${cursor.toLocaleDateString("en", { month: "short", day: "numeric" })}`,
          momo: weekOrders.filter((o) => o.status === "completed" && o.payment_method === "momo").reduce((s, o) => s + o.total, 0),
  ...
  const transactions = [...rangeOrders].sort((a, b) => new Date(b.created_date) - new Date(a.created_date)).slice(0, 100);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        <Card className="p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-heading font-semibold text-sm">Select Date Range</h3>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 sm:items-end">
            <div className="grid gap-1.5">
              <label className="text-xs text-muted-foreground">From</label>
              <Input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="h-9 w-full sm:w-44" />
            </div>
            <div className="grid gap-1.5">
              <label className="text-xs text-muted-foreground">To</label>
              <Input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="h-9 w-full sm:w-44" />
            </div>
            <div className="flex flex-wrap gap-1.5">
              {[
                ...
                <Button key={p.key} variant="outline" size="sm" className="h-9 text-xs" onClick={() => applyPreset(p.key)}>
                  {p.label}
                </Button>
              ))}
            </div>
                  ...
                  <div key={o.id} className="flex items-center justify-between py-3 gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        {new Date(o.created_date).toLocaleString("en", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                        {" · "}
                        ...
                        <span className={
                          ...
                          o.status === "preparing" ? "text-blue-600" : "text-violet-600"
                        }>{o.status}</span>
                      </p>
                    </div>
