...
import { ArrowLeft, Loader2, Receipt, Clock, Smartphone, Banknote, Phone, Hash } from "lucide-react";
...
function formatDate(dateStr) {
  return new Date(dateStr).toLocaleString("en", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" });
}
...
export default function OrderHistory() {
  ...
  useEffect(() => {
    if (isStaff) {
      ...
      api.get(`/api/orders?client_phone=${encodeURIComponent(client.phone_number)}`)
        .then((res) => setOrders(res.data))
        .catch(() => {})
        .finally(() => setLoading(false));
    } else {
              ...
              return (
                <Card key={order.id} className="p-4">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {order.client_phone && <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground"><Phone className="w-3 h-3" /> {order.client_phone}</span>}
                  </div>
                  <div className="space-y-1">
                    {order.items?.map((item, i) => (
                      <div key={i} className="flex items-center justify-between text-sm"><span className="text-muted-foreground">{item.quantity}x {item.name}</span><span>{"\u20B5"}{(item.price * item.quantity).toFixed(2)}</span></div>
                    ))}
                  </div>
                  {order.pickup_note && <p className="text-xs text-muted-foreground mt-2 pt-2 border-t">Note: {order.pickup_note}</p>}
                </Card>
              );
            })}
          </div>
