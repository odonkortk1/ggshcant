...
export default function MoMoDialog({ open, onOpenChange, total, items, customerName, pickupNote, clientPhone, onSuccess }) {
  const [phone, setPhone] = useState("0597394449");
  ...
  const submitPayment = async () => {
    ...
    if (window.self !== window.top) {
      setError("Checkout works only from the published app. Please open the app directly.");
      return;
    }
    setLoading(true);
    try {
      const response = await base44.functions.invoke("paystack-initialize", {
        items,
      ...
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 font-heading">
            <Smartphone className="w-5 h-5 text-blue-600" /> Mobile Money Payment
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="grid gap-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              You'll be redirected to Paystack to approve the mobile money payment on your phone.
            </p>
          </div>
        ...
        <DialogFooter>
          <Button
            ...
            disabled={!/^0\d{9}$/.test(phone) || loading}
