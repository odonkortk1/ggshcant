    ...
    <div className={`rounded-2xl bg-card border shadow-sm p-4 ring-1 ${cfg.ring} flex flex-col gap-3`}>
      ...
      <div className="text-xs text-muted-foreground space-y-0.5">
        {order.items?.slice(0, 3).map((it, i) => (
          <div key={i} className="flex justify-between"><span>{it.quantity}× {it.name}</span><span>₵{(it.price * it.quantity).toFixed(2)}</span></div>
        ))}
        {order.items?.length > 3 && <p className="text-[11px] italic">+{order.items.length - 3} more</p>}
      </div>

      {order.pickup_note && <p className="text-[11px] bg-muted/50 rounded-md px-2 py-1 text-muted-foreground">📝 {order.pickup_note}</p>}
