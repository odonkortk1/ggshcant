-- Mirrors the entities extracted from base44/entities/*.jsonc
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  items TEXT NOT NULL, -- JSON array: [{name, price, quantity}]
  total REAL NOT NULL,
  status TEXT NOT NULL DEFAULT 'pending' CHECK(status IN ('pending', 'preparing', 'ready', 'completed')),
  pickup_note TEXT,
  payment_method TEXT DEFAULT 'momo' CHECK(payment_method IN ('momo', 'cash', 'ggsh_ussd', 'stripe', 'paystack')),
  client_phone TEXT,
